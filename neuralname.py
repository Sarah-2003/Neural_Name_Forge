...
class LSTMCell(nn.Module):
    """
    LSTM cell implementation following Graves et al. 2014
    https://arxiv.org/abs/1308.0850
    """
    def __init__(self, config):
        super().__init__()
        # Gates: input, forget, output, and cell state
        ...
        self.xh_to_o = nn.Linear(config.n_embd + config.n_embd2, config.n_embd2)
        self.xh_to_c = nn.Linear(config.n_embd + config.n_embd2, config.n_embd2)
        # Initialize forget gate bias to 1 to encourage remembering
        self.xh_to_f.bias.data.fill_(1.0)
    ...
    def forward(self, xt, state):
        ...
        # Calculate gates
        i = F.sigmoid(self.xh_to_i(xh))  # input gate
        f = F.sigmoid(self.xh_to_f(xh))  # forget gate
        o = F.sigmoid(self.xh_to_o(xh))  # output gate
        
        # Calculate new cell state candidate
        c_tilde = F.tanh(self.xh_to_c(xh))
        
        # Update cell state
        ct = f * cprev + i * c_tilde
...
class RNN(nn.Module):
    def __init__(self, config, cell_type):
        ...
        self.block_size = config.block_size
        self.vocab_size = config.vocab_size
        self.start_h = nn.Parameter(torch.zeros(1, config.n_embd2))
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        ...
        if cell_type == 'rnn':
            ...
            self.start_c = nn.Parameter(torch.zeros(1, config.n_embd2))
        self.cell_type = cell_type
        self.lm_head = nn.Linear(config.n_embd2, self.vocab_size)

    def get_block_size(self):
        """Return the block size (context length) that this model expects."""
        return self.block_size
    ...
    def forward(self, idx, targets=None):
        ...
        # initialize states
        hprev = self.start_h.expand((b, -1))
        state = hprev if self.cell_type != 'lstm' else (hprev, self.start_c.expand((b, -1)))
        
        hiddens = []
        for i in range(t):
            xt = emb[:, i, :]  # (b, n_embd)
            if self.cell_type == 'lstm':
                ht, ct = self.cell(xt, state)
                state = (ht, ct)
                hiddens.append(ht)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', type=str, default='transformer', 
                       help="model class type to use, bigram|mlp|rnn|gru|lstm|bow|transformer")
